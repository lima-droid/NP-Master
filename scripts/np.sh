...
E[2]="The script must be run as root, you can enter sudo -i and then download and run again. Feedback: [https://github.com/lima-droid/NP-Master/issues]"
C[2]="必须以 root 方式运行脚本，可以输入 sudo -i 后重新下载运行，问题反馈:[https://github.com/lima-droid/NP-Master/issues]"
...
C[84]="选择 np-master 内核（安装后可用 [np -t] 切换）：1. 稳定（v1.16.0） 2. 最新（v1.17.0） 3. 经典（v1.10.3）"
...
C[93]="最新版可以从 \$DEV_LOCAL_VERSION 升级到新版本"
...
C[120]="创建备份文件失败"
E[121]="Cleaning up offline directory..."
C[121]="清理离线目录..."
E[122]="Offline directory cleaned successfully"
C[122]="离线目录清理成功"
# 自定义字体彩色，read 函数
...
get_intranet_penetration_server_cmd() {
  ...
  if [[ "$CLIENT_CMD" =~ \"url\":[[:space:]]*\"client://([^\@]*)@?([0-9]+\.[0-9]+\.[0-9]+|\[[0-9a-fA-F:]+\]):([0-9]+)/[0-9]+\.[0-9]+\.[0-9]+:[0-9]+\" ]]; then
...
install() {
  ...
  if [ $INSTALL_STATUS -eq 0 ]; then
    ...
    warning " $(text 53) "
    # 如果安装失败，也尝试清理离线目录（可选，这里为了保持环境整洁）
    cleanup_offline_directory
  fi
  help
}
